...
class _TabLancamentoListaTelaState extends State<TabLancamentoListaTela> {
  ...
  List<LancamentoModel> _lancamentoLista = [];
  String corLancamentoDespesa = "";
  String corLancamentoReceita = "";
  // CALENDARIO
  List<CalendarioModel> _calendarioLista = [];
  ...
  _thumb(LancamentoModel item , context){
    //DATA
    DateTime dataSelecionada = item.frequencia!.lista![0].frequenciaData as DateTime;
    // DIA
    int dia = Funcoes.retornarDiaDataEUA( dataSelecionada );
    String mes = Funcoes.retornarMesDataEUA( dataSelecionada);
    String corTipoLancamento = (item.tipo!.id == 1)?corLancamentoDespesa:corLancamentoReceita;

    // VALORES
    // SE FOR DESPESA
    if(item.tipo!.id == 1){
      _valorTotalDespesa = _valorTotalDespesa + double.parse(item.frequencia!.lista!.first.valor.toString());
      _valorTotalSaldo = _valorTotalSaldo - double.parse(item.frequencia!.lista!.first.valor.toString() );
    }else{
      _valorTotalReceita = _valorTotalReceita + double.parse(item.frequencia!.lista!.first.valor.toString() );
      _valorTotalSaldo = _valorTotalSaldo + double.parse(item.frequencia!.lista!.first.valor.toString() );
    }

    var icone = new IconeCadModel.find(item.categoria!.icone.toString());
    IconeGambiarra categoriaIcone = (icone !=null)
    ? new IconeGambiarra( icone , Colors.white, 22,) 
    : IconeGambiarra( new IconeCadModel.find('help-01') , Colors.white, 22,);

    String descricao = (item.descricao.toString().isNotEmpty)
    ...
    if(item.pagamento!.cartao == null){
      pagamentoLabel = item.pagamento!.pagamentoForma!.descricao.toString();
    }else{
      // 20210524
  ...
  _dataSelecionada(String opcao) {
    ...
    if(opcao == "posterior-mes"){
      dataPesquisar = _calendarioLista[2].data as DateTime;
    ...
    if(opcao == "anterior-mes"){
      dataPesquisar = _calendarioLista[0].data as DateTime;
      _calendarioLista = _calendario.gerarLista( dataPesquisar );
    }
...
//    }
  ...
  Future <List<LancamentoModel>> _buscarLancamentos() async {
    int ano = _calendarioLista[1].ano as int;
    int mes =_calendarioLista[1].mes as int ;
