...
class _LancamentoDespesaTelaState extends State<LancamentoDespesaTela> {
  ...
  List<IconeCadModel> _listaIcones = [];

  // TIPO
  LancamentoTipoCadModel _tipo = new LancamentoTipoCadModel();
  List<LancamentoTipoCadModel> _listaTipo = [];
  ...
  List<CategoriaCadModel> _listaCategoria = [];
  String _categoriaDescricaoController = "";
  IconeGambiarra ?categoriaIcone;
  ...
  List<FrequenciaCadModel> _listaFrequencia = [];
  List<FrequenciaPeriodoCadModel> _listaFrequenciaPeriodo = [];
...
//      _frequenciaDescricaoController = _frequenciaCad.descricao;
  ...
  _definirCores() {
    ...
    setState(() {
      ...
      String modo = _dados['modo'];
      _background = Funcoes.converterCorStringColor( Configuracoes.cores[modo]!['background'].toString() );
      _colorFundo = Funcoes.converterCorStringColor( Configuracoes.cores[modo]!['background'].toString() );
      _colorContainerFundo = Funcoes.converterCorStringColor( Configuracoes.cores[modo]!['containerFundo'].toString() );
      _colorContainerBorda = Funcoes.converterCorStringColor( Configuracoes.cores[modo]!['containerBorda'].toString() );
      _colorLetra = Funcoes.converterCorStringColor(Configuracoes.cores[modo]!['textoPrimaria'].toString() );
    });
    return;
  }
...
//                               child: Icon(Icons.edit),
                               child: IconeGambiarra( 
                                 new IconeCadModel(
                                   icone: 'carrinho-01' , 
                                   codigo: 0xf0110, 
                                   familia:'Material Design Icons' , 
                                   pacote: 'material_design_icons_flutter',
                                  )
                                  ,Colors.black
                                  , 22,
                                ),
                    ...
                    for(PagamentoFormaCadModel objeto in _pagamentoFormaLista) Column(
                      children: [
                        Padding(
                          padding: EdgeInsets.all(10.0),
                          child: FlatButton(
                            child: Text( objeto.descricao.toString() ,style: TextStyle(color: Theme.of(context).primaryColor, fontSize: 20.0) ),
                            onPressed: (){
  ...
  _setarCategoria(CategoriaCadModel ?objeto) async {
    var icone;
    if(objeto !=null){
      _categoriaDescricaoController = objeto.descricao.toString();
      icone = new IconeCadModel.find( objeto.icone.toString() );
      categoriaIcone = IconeGambiarra( icone , Colors.black, 22,);
  ...
  _setarPagamento(PagamentoFormaCadModel ?objeto) async {
    ...
    if(objeto != null){
      icone = new IconeCadModel.find(objeto.icone.toString());
      pagamentoIcone = IconeGambiarra( icone , Colors.black, 22 );
      pagamentoDescricaoController = objeto.descricao.toString();
  ...
  _setarFrequencia(FrequenciaCadModel ?frequencia , objeto) async {
    ...
    if( frequencia !=null && objeto != null){
      ...
      _frequenciaDescricaoController = "${frequencia.descricao}(${quantidade}) - ${ periodo.descricao.toString()}";
    }else{
