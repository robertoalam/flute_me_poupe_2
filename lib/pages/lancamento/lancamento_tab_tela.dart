...
class _LancamentoTabTelaState extends State<LancamentoTabTela>  with SingleTickerProviderStateMixin {
  ...
  List<CategoriaCadModel> listaCategoria = [];
  CartaoModel _cartao = new CartaoModel();
  List<CartaoModel> _listaCartao = [];

  FrequenciaPeriodoCadModel _frequenciaPeriodo = new FrequenciaPeriodoCadModel();
  List<FrequenciaPeriodoCadModel> _frequenciaPeriodoLista = [];
...
class _CirclePainter extends BoxPainter {
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration cfg) {
    double widthNew = cfg.size?.width as double;
    double heigthNew = cfg.size?.height as double;
    final Offset circleOffset = offset + Offset( widthNew  / 2, heigthNew - radius - 5);
