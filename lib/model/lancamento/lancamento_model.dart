...
class LancamentoModel{
  ...
  Future<int> insert() async {
    ...
    try{
     ...
     if( id > 0 ) {
       ...
       await lancamentoPagamentoSalvar(
         id , 
         this.chaveUnica.toString() ,
         this.pagamento as PagamentoModel
        );
       await lancamentoFrequenciaSalvar(
         id , 
         this.chaveUnica.toString() ,
         this.frequencia as FrequenciaModel
        );
       await lancamentoFrequenciaDetalheSalvar(
         id , 
         this.chaveUnica.toString() ,
         this.data as DateTime, 
         this.frequencia as FrequenciaModel
        );
  ...
  fetchByYearAndMonth(int ano , int mes , {Map ?where , String ?order}) async {
    ...
    for(var linha in linhas){
      ...
      if(linha.isNotEmpty){
        ...
        List<FrequenciaDetalheModel> listagem = [];
        // FREQUENCIA DETALHE
  ...
  lancamentoFrequenciaDetalheSalvar(int idLancamento ,String chaveUnica, DateTime dataLancamento,FrequenciaModel objeto) async {
    ...
    int numeroTotalPeriodos = int.parse( objeto.quantidade.toString() );
    
    double valorLancamento = (frequencia!.frequencia!.id == 2)
      ? double.parse( frequencia!.valorIntegral.toString() )
      : double.parse(frequencia!.valorIntegral.toString()) / numeroTotalPeriodos;

    int numeroDias = int.parse( objeto.frequenciaPeriodo!.dias.toString() );
    // [REGRA DE NEGOCIO]
