...
//    }).toList();
  ...
  bancoCadPopular(Database db) async{
    ...
    lista.map( (objeto) async {
      ...
      int id = int.parse(objeto.id.toString());
      String descricao = objeto.descricao.toString();
      String corPrimaria = ( objeto.corPrimaria != null ) ? objeto.corPrimaria.toString() : "#FFFFFF";
      String corSecundaria = ( objeto.corSecundaria != null ) ? objeto.corSecundaria.toString() : "#FFFFFF";
      String corTerciaria = ( objeto.corTerciaria != null ) ? objeto.corTerciaria.toString() : "#FFFFFF";
      String corCartao = ( objeto.corCartao != null ) ? objeto.corCartao.toString() : "#FFFFFF";
      String imagemAsset = ( objeto.imageAsset != null ) ? objeto.imageAsset.toString() : "assets/images/bancos/logo/00.png";
      int destaque = (int.parse(objeto.destaque.toString() )==1)? 1 : 0;
      int ordemDestaque = int.parse(objeto.ordemDestaque.toString());
  ...
  categoriaCadPopular(Database db) async{
    ...
    lista.map( (objeto) async {
      int id = int.parse(objeto.id.toString());
      int id_pai = int.parse(objeto.idPai.toString());
      String descricao = objeto.descricao.toString();
      String icone = objeto.icone.toString();
      int ordem = int.parse(objeto.order.toString() );
      String query = "INSERT INTO "+CategoriaCadModel.TABLE_NAME+" ( _id , id_pai , descricao , icone , ordem) VALUES ("+id.toString()+" ,"+id_pai.toString()+", '"+descricao.toString()+"' , '"+icone.toString()+"',"+ordem.toString()+");";
      await db.execute(query);
    }).toList();
  }
  ...
  seedPopularLancamentos(Database db) async {
    ...
    linhas.map( (linha) async {
      ...
      double linhaFrequenciaDetalheValor = (linha['id_frequencia'] == 2)?  double.parse( linha['vl_integral'] ) : double.parse( linha['vl_integral'] / linhaFrequencia['no_quantidade'] );
      int salto;
...
// Helper methods
  ...
  Future<List<Map<String, dynamic>>> executarQuery(String query,{List ?where , String order= "" , String limit = ""}) async {
    ...
    dynamic list = await db.rawQuery(query);
    return list;
  }
  ...
  Future<List<Map<String, dynamic>>> queryCustom(sql) async {
    ...
    dynamic list = await db.rawQuery(sql);
    return list;
  }
